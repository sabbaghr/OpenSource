...
/**
 * writeSnapTimesReset -- Reset the first-call flag so the next
 * writeSnapTimes call creates (truncates) files instead of appending.
 * Call this before writing a new set of snapshots to a different
 * file base name (e.g., adjoint wavefield snapshots).
 */
void writeSnapTimesReset(void)
{
	extern int _writeSnap_first;
	_writeSnap_first = 1;
}

int _writeSnap_first = 1;
...
int writeSnapTimes(modPar mod, snaPar sna, bndPar bnd, wavPar wav, int ixsrc, int izsrc, int itime, float *vx, float *vz, float *tzz, float *txx, float *txz, int verbose)
{
	...
	if ( (((itime-sna.delay) % sna.skipdt)==0) && 
		...
		if (verbose>1) vmess("Writing snapshot(%d) at time=%.4f", isnap+1, stime+sna.t0);
