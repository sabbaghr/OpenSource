...
/*			vmess("Memory requirement for sources = %.2f MB.",sizeof(float)*(wav->nx*(wav->nt/(1024.0*1024.0))));*/
	...
	rec->nt=MIN(rec->nt, NINT(ceil(((mod->tmod-rdelay+0.01*mod->dt)/dtrcv))));
...
*/
	...
	if (!getparint("rec_type_q", &rec->type.q)) rec->type.q=0;
	if (mod->ischeme!=1 &&  rec->type.ud==1) {
		warn("Receiver decomposition only implemented for acoustis scheme (1)");
	}
	if (mod->ischeme>2) {
		/* For elastic: rec_type_p records true pressure = 0.5*(Tzz + Txx) */
		if (!getparint("rec_type_p", &rec->type.p)) rec->type.p=0;
	...
	if (verbose && pe==0) {
		if (rec->n) {
			...
			if (rec->type.q) fprintf(stderr,"q ");
