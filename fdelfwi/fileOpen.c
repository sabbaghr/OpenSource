...
//    fprintf(stderr,"PE %d open file %s\n", pe, file);
    ...
    /* Set open mode based on append flag */
    if (append) {
        mode = MPI_MODE_APPEND | MPI_MODE_CREATE | MPI_MODE_WRONLY;
    } else {
        mode = MPI_MODE_CREATE | MPI_MODE_WRONLY;
    }
    result = MPI_File_open(MPI_COMM_WORLD, filename, mode, MPI_INFO_NULL, &fh);
