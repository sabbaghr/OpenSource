#!/bin/bash
#
# test_bandpass_filter.sh - Verify wavelet FFT filter matches sufilter.
#
# Creates a Ricker wavelet, filters it two ways:
#   1. Our bandpass_filter_wavelet (FFT-based, via test program)
#   2. sufilter (Seismic Unix)
# Then compares the spectra and time-domain traces.
#
# If the filters match, the difference should be ~zero (machine epsilon).
# If artifacts appear, this reveals the mismatch.
#
set -e
...
echo "=== Building test_bandpass_filter ==="
cat > test_bandpass_filter.c << 'CEOF'
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
#include <string.h>
...
# Compile
gcc -I../include -I. -I$MKLROOT/include -I$MKLROOT/include/fftw \
    -DWISDOMDIR=\"./\" -O3 -o test_bandpass_filter \
    test_bandpass_filter.c bandpass_filter.o verbosepkg.o \
    atopkge.o docpkge.o getpars.o \
    -L../lib -lgenfft \
    -L$MKLROOT/lib/intel64 -lmkl_intel_lp64 -lmkl_gnu_thread -lmkl_core \
    -lgomp -lpthread -lm -ldl

echo ""
echo "=== Test 1: Bandpass [2, 8] Hz on 10 Hz Ricker ==="
./test_bandpass_filter 2.0 8.0 10.0

echo ""
echo "=== Test 2: Bandpass [2, 5] Hz on 5 Hz Ricker ==="
./test_bandpass_filter 2.0 5.0 5.0

echo ""
echo "=== Test 3: Bandpass [2, 20] Hz on 10 Hz Ricker ==="
./test_bandpass_filter 2.0 20.0 10.0

echo ""
echo "=== Test 4: Bandpass [1, 5] Hz (narrow) on 10 Hz Ricker ==="
./test_bandpass_filter 1.0 5.0 10.0

# Clean up build artifacts
rm -f test_bandpass_filter test_bandpass_filter.c
