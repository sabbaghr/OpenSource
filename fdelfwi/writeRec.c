             ...
             float *rec_p, float *rec_pp, float *rec_ss, float *rec_q, float *rec_udp, float *rec_udvz, float *rec_dxvx, float *rec_dzvz, int verbose)
{
    ...
    /* For FWI: always add shot number suffix to keep files separate per shot */
    strcpy(filename, rec.file_rcv);
    if (fileno >= 0) {
        sprintf(number,"_%03d",fileno);
        name_ext(filename, number);
        /* Each shot gets its own file, so never append */
        append = 0;
#ifdef MPI
        /* For FWI: close previous files and reset flag to open new files */
        if (opened) {
            ...
            if (rec.type.q) fileClose(fpq);
            opened = 0;
        }
#endif
    }
    else {
        /* Traditional mode: append shots to same file */
        if (ishot) append=1;
        else append=0;
    }
    ...
    hdr.ns     = nsam;
    hdr.trwf   = rec.n;
    /* For FWI mode (fileno >= 0), each shot has its own file with rec.n traces.
     * For traditional mode, all shots go in one file with nshots*rec.n traces. */
    hdr.ntr    = (fileno >= 0) ? rec.n : nshots*rec.n;
