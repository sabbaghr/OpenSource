	...
	for (irec=0; irec<rec.n; irec++) {
		...
		if ( rec.int_p==3 ) {
			...
			if (rec.type.p){
				 ...
				 * For elastic (ischeme > 2): P = 0.5*(Tzz + Txx)
				 * For acoustic: P = Tzz */
				if (mod.ischeme > 2 && txx) {
					C00 = 0.5*(tzz[ix*n1+iz]       + txx[ix*n1+iz]);
					C10 = 0.5*(tzz[(ix+1)*n1+iz]   + txx[(ix+1)*n1+iz]);
					C01 = 0.5*(tzz[ix*n1+iz+1]     + txx[ix*n1+iz+1]);
					C11 = 0.5*(tzz[(ix+1)*n1+iz+1] + txx[(ix+1)*n1+iz+1]);
				} else {
					C00 = tzz[ix*n1+iz];
					C10 = tzz[(ix+1)*n1+iz];
					C01 = tzz[ix*n1+iz+1];
			...
			if (rec.type.p) {
				if (rec.int_p == 1) {
					...
					/* No time interpolation: direct grid point read
					 * For elastic: P = 0.5*(Tzz + Txx) */
					if (mod.ischeme > 2 && txx)
						rec_p[irec*rec.nt+isam] += 0.5*(tzz[ix*n1+iz] + txx[ix*n1+iz]);
					else
			...
			if (rec.type.dxvx) {
                rec_dxvx[irec*rec.nt+isam] = (c1*(vx[(ix+1)*n1+iz] - vx[ix*n1+iz]) +
                       c2*(vx[(ix+2)*n1+iz] - vx[(ix-1)*n1+iz]))/mod.dx;
            }
			if (rec.type.dzvz) {
			...
			if (rec.type.q) {
				rec_q[irec*rec.nt+isam] += q[ix*n1+iz];
            }
			if (rec.type.vz) {
